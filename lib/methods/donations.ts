...
export class DonationEngine {
    ...
    public async createDonationByWebhook(stripeChargeId: string) {
        ...
        try {
            const donationData = await this.fetchDonationFromStripe({
                stripeChargeId
            });

            if (!donationData) {
                throw new Error("Failed to fetch donation data from Stripe");
            }

            console.log(`[DonationEngine] Successfully fetched donation data from Stripe: ${donationData.id}`);
            ...
            try {
                ...
                if (existingDonation) {
                    console.log(`[DonationEngine] Donation with ID ${donationData.id} already exists, skipping insertion`);
                    return existingDonation;
                }
                
                const savedDonation = await this.insertDonationObject(donationData);
    ...
    private async fetchDonationFromStripe(props: BuildDonationFromStripeProps) {
        let paymentIntentObject: Stripe.PaymentIntent | undefined = undefined;
        let chargeObject: Stripe.Charge | undefined = undefined;
        let balanceTransactionObject: Stripe.BalanceTransaction | undefined = undefined;
        let customerObject: Stripe.Customer | undefined = undefined;

        // Only fetch Stripe data if we have a Stripe ID
        ...
        // For Stripe donations, use metadata from charge object
        if (chargeObject) {
            ...
            return {
                ...
                stripe_customer_id: customerObject?.id || null,
        ...
        // If no Stripe data, return null to let the caller handle the donation data
        return null;
    }
    ...
    async saveCauseForDonation(donation_id: string, causeData: any) {
        ...
        const dataToCreate: any = {
            ...
            amount_cents: causeData.amount_cents,
